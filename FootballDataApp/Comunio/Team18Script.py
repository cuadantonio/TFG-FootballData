...
team = "Sevilla"
url = "https://www.comuniazo.com/comunio-apuestas/equipos/sevilla"
...
for eachPlayer in allPlayers:
    ...
    if(len(nicknameList)!=1):
        ...
        playerNameSplit = str(playerName).split(" ")
        newPlayerName = playerNameSplit[0]+" "+playerNameSplit[2]
        query3 = comunio.find_one(
            {"name": {"$regex": re.compile(newPlayerName, re.IGNORECASE)}, "team": team, "comunioFound": False})
        if query3 != None:
            playerUpdate = {"comunioPoints": query3["points"], "comunioPrice": query3["price"]}
            newvalues3 = {"$set": playerUpdate}
            playerUpdateComunio = {"comunioFound": True}
            newValuesComunio = {"$set": playerUpdateComunio}
            playersRealData.update_one(eachPlayer, newvalues3)
            comunio.update_one(query3, newValuesComunio)
            continue