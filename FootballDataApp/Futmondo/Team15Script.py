...
team = "Rayo Vallecano"
url = "https://www.futmondo.com/team?team=504e581e4d8bec9a670000ca"
...
for eachPlayer in allPlayers:
    ...
    if(len(nicknameList)!=1):
        ...
        nameAux = nameList[0]
        query4 = futmondo.find_one(
            {"name": {"$regex": re.compile(nameAux, re.IGNORECASE)}, "team": team, "futmondoFound": False})
        if query4 != None:
            playerUpdate = {"futmondoPoints": query4["points"], "futmondoPrice": query4["price"]}
            newvalues4 = {"$set": playerUpdate}
            playerUpdateFutmondo = {"futmondoFound": True}
            newValuesFutmondo = {"$set": playerUpdateFutmondo}
            playersRealData.update_one(eachPlayer, newvalues4)
            futmondo.update_one(query4, newValuesFutmondo)
            continue