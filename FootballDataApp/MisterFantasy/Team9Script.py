...
team = "Espanyol"
url = "https://mister.mundodeportivo.com/teams/8/"
...
for eachPlayer in allPlayers:
    ...
    if (len(nicknameList) != 1):
        ...
        nameAux = nameList[0]+" "+nameList[1]+" "+nameList[2]
        query4 = misterfantasy.find_one(
            {"name": {"$regex": re.compile(nameAux, re.IGNORECASE)}, "team": team, "misterFantasyFound": False})
        if query4 != None:
            playerUpdate = {"misterFantasyPoints": query4["points"], "misterFantasyPrice": query4["price"]}
            newvalues4 = {"$set": playerUpdate}
            playerUpdateMisterFantasy = {"misterFantasyFound": True}
            newValuesMisterFantasy = {"$set": playerUpdateMisterFantasy}
            playersRealData.update_one(eachPlayer, newvalues4)
            misterfantasy.update_one(query4, newValuesMisterFantasy)
            continue
