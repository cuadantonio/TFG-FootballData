...
for i in pages:
    ...
    headers = {
        ...
        'x-rapidapi-key': "7550afb51emshfbd1034aea3674ep1ec6cfjsn1fd7885fe38c"
    }
    ...
    for j in range(len(jsonResponse)):
        ...
        if query2 != None:
            ...
            continue
        query5 = collection.find_one({"name":{"$regex":re.compile(lastname, re.IGNORECASE)},"team":team,"teamId":teamId})
        if query5 != None:
            query5Filter = {"name":query5["name"]}
            newvalues5 = {"$set": playerUpdate}
            collection.update_one(query5Filter, newvalues5)
            continue
        query6 = collection.find_one({"nickname":{"$regex":re.compile(lastname, re.IGNORECASE)},"team":team,"teamId":teamId})
        if query6 != None:
            query6Filter = {"name":query6["name"]}
            newvalues6 = {"$set": playerUpdate}
            collection.update_one(query6Filter, newvalues6)
            continue
        query7 = collection.find_one({"name":{"$regex":re.compile(fullname, re.IGNORECASE)},"team":team,"teamId":teamId})
        if query7 != None:
            query7Filter = {"name":query7["name"]}
            newvalues7 = {"$set": playerUpdate}
            collection.update_one(query7Filter, newvalues7)
            continue
        query8 = collection.find_one({"nickname":{"$regex":re.compile(fullname, re.IGNORECASE)},"team":team,"teamId":teamId})
        if query8 != None:
            query8Filter = {"name":query8["name"]}
            newvalues8 = {"$set": playerUpdate}
            collection.update_one(query8Filter, newvalues8)
            continue
