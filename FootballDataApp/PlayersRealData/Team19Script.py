...
a, b = 'áãàéíóøúüćčşÁÉÍÓÚÜ-', 'aaaeioouuccsAEIOUU '
...
for i in pages:
    ...
    headers = {
        ...
        'x-rapidapi-key': "fa7879edfdmsh988da57ba135a9bp138896jsn7b87938e4af0"
    }
    ...
    for j in range(len(jsonResponse)):
        ...
        query2 = collection.find_one(
            {"nickname": {"$regex": re.compile(lastname, re.IGNORECASE)}, "team": team, "teamId": teamId})
        if query2 != None:
            query2Filter = {"nickname": {"$regex": re.compile(lastname, re.IGNORECASE)}, "team": team, "teamId": teamId}
